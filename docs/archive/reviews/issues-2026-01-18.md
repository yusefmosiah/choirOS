...
- [x] BAML Phase 1 | Verifier Walking Skeleton: `VerifierRunner` uses BAML for structured analysis. Files: `baml_src/verifier.baml`, `supervisor/verifier_runner.py`.
- [x] BAML Phase 2 | Tooling & Agents: Tool schemas + agent functions. Files: `baml_src/tools.baml`, `baml_src/agent.baml`.
- [x] BAML Phase 3 | The Director: `RunOrchestrator` uses BAML `AssessTask` for intelligent verification gating.
