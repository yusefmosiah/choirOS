...
## Open
- [ ] VC-01 | High | Version control primitives for supervisor workspaces: create checkpoints, diff, and rollback on verifier failure. Spec drafted: `docs/specs/CHOIR_STORAGE_ROLLBACK_SPEC.md`.
- [ ] SBX-01 | High | Sandbox lifecycle + isolation via sprites.dev: create/tear down, resource limits, network policy, snapshot/restore hooks. Spec drafted: `docs/specs/CHOIR_STORAGE_ROLLBACK_SPEC.md`.
- [ ] SBX-02 | Medium | sprites.dev adapter: API/CLI wrapper for create, exec, checkpoint, restore, proxy; token management + audit.
- [ ] FE-01 | High | Host React frontend inside inner sandbox; enable supervisor-triggered rollback when bad code breaks the UI.
...
- [ ] OS-01 | Medium | OS Connector: mount Choir as drive (Windows TBD, macOS File Provider/FUSE, Linux FUSE) — split into coordinator + platform subagents.
