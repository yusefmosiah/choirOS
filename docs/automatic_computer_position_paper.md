...
```text
ActionEvent { timestamp, action_type, // "file_changed", "email_sent", "app_opened" target, // what was acted upon context, // surrounding state }
```
...
```sql
CREATE TABLE workspace ( id TEXT PRIMARY KEY, content BLOB, metadata JSON, updated_at TIMESTAMP );
CREATE TABLE action_log ( id TEXT PRIMARY KEY, action_type TEXT, target TEXT, context JSON, timestamp TIMESTAMP );
CREATE TABLE agent_tasks ( id TEXT PRIMARY KEY, status TEXT, -- pending, running, complete
  input JSON, output JSON, created_at TIMESTAMP, completed_at TIMESTAMP );
```
