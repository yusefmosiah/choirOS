...
This document provides a high-level overview of the ChoirOS architecture, capturing the relationships between the Supervisor, API, ChoirOS Frontend, NATS, and Sprites.
...
graph TD
    ...
    Supervisor -->|JetStream| NATS[NATS Message Bus]
...
- **Key Responsibilities**:
    ...
    - **Artifacts**: `api.routers.artifacts` (likely for retrieving/storing static assets).
...
- **Usage**: Used by Supervisor to store the "Stream of Consciousness" (events) or command logs. This enables the event-sourced architecture where state can be rebuilt from events.

### 5. Sprites (`supervisor/sprites_adapter.py`)
...
5.  **Starts Frontend**: Runs `npm run dev` in `choiros/` on port 5173.
