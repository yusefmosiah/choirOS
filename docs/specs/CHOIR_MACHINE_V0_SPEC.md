...
## Decision summary
- SQLite is the canonical event store for local development; NATS is used for optional replication and transport.
...
## 2) Core invariants
1) SQLite is the source of truth for local dev; all state can be rebuilt from the event log.
